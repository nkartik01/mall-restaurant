...
export default class OperatorEdit extends Component {
  ...
  getOperator = async (e) => {
    var res = await axios.get("http://192.168.2.2:5001/mall-restraunt/us-central1/api/operator/getOperator/" + this.props.match.params.username, {
  ...
  onSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://192.168.2.2:5001/mall-restraunt/us-central1/api/operator/edit", data, { headers: { "x-auth-token": localStorage.getItem("token") } });
