...
export default class BillList extends Component {
  ...
  getBills = async () => {
    var bills = await axios.get("http://192.168.2.2:5001/mall-restraunt/us-central1/api/bill/listBills/50", { headers: { "x-auth-token": localStorage.getItem("token") } });
  ...
  getPending = async (e) => {
    e.preventDefault();
    var bills = await axios.get("http://192.168.2.2:5001/mall-restraunt/us-central1/api/bill/pendingBills", { headers: { "x-auth-token": localStorage.getItem("token") } });
    bills = bills.data.bills;
    this.setState({ bills });
    this.props.store.dispatch(setData({ bills }));
  };
